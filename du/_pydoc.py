...
def safeimport(path, forceload=0, cache={}):
    ...
    try:
        ...
        elif (exc is ImportError or exc is ModuleNotFoundError)\
                and value.name == path:
...
""")
        ...
        if data:
            contents = []
            for key, value in data:
                contents.append(self.docother(value, key, name, maxlen=60))
    ...
    def docroutine(self, object, name=None, mod=None, cl=None):
        ...
        #user_meths = [k: v for k, v in cl.__dict__.items() if not k.startswith('__')}
        #user_meths = [k for k in cl.__dict__.keys() if not k.startswith('__')]
        #print(name, realname, mod, cl, type(cl))
        #quit()
        #if mod is not None and cl is None and not name in user_meths:
        #print(mod is not None)
        #quit()
        #if mod is not None and cl is None and not name in user_meths:
        if cl is not None:
          #if name not in [k for k in cl.__dict__.keys() if not k.startswith('__')]:
          if name not in cl.__dict__.keys():
            skipdocs = 1
